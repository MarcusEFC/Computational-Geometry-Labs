 ...
 *     This function iterates through all the vector of points representing the triangulated polygon, and uses OpenGL on all vectors,
 *     rendering a corresponding triangle for each vector.
 */
 ...
 *
 * Return Values:
 *     A bool indicating whether or not the current direction of the guard is a valid direction, i.e. is inside the polygon.
 *
 * Functional Description:
 *     This function takes the current position of the guard point, and adds the current direction of the guard, and then determines
 *     if the new position after the direction is added is inside the polygon.
 */

bool currDirectionValid() {

  point2D testPoint;
  testPoint.x = guardPoint.x + currentDirectionX;
  testPoint.y = guardPoint.y + currentDirectionY;
 ...
 *
 * Functional Description:
 *     This continues to change the direction of the guard, while the current direction of the guard is invalid.
 */
...
void determineNewDirection() {
  ...
  while (!currDirectionValid()) {
    ...
    while (currentDirectionX == 0 && currentDirectionY == 0) {
      currentDirectionX = randomDirection();
 ...
 *
 * Functional Description:
 *     While the moveGuard bool is true, this function will be continuously called, thus moving the guard in the current
 *     direction of the guard, and then generating visible polygon with respect to the guard in the new position.
 */
...
void timerfunc() {
  ...
  if (moveGuard == true) {
    ...
    if (!isInPolygon(poly, guardPoint) || !isSimple(poly)) {
      exit(0);
    }

    if (currDirectionValid()) {

      guardPoint.x = guardPoint.x + currentDirectionX;
      guardPoint.y = guardPoint.y + currentDirectionY;

      mouse_x = guardPoint.x;
      mouse_y = guardPoint.y;
