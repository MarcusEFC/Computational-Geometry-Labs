...
vector< vector<point2D> > triangulate, triangulate2;
point2D guardPoint1, guardPoint2;
...
int two_guard_mode = 0;

int guard1Initialized, guard2Initialized = 0;
...
void draw_guard1(){
  ...
  for(int i = 0; i < precision; i++){
    theta = i * 2 * M_PI/precision;
    glVertex2f(guardPoint1.x + r*cos(theta), guardPoint1.y + r*sin(theta));
  }
  glEnd();
}
...
void draw_guard2(){
  ...
  for(int i = 0; i < precision; i++){
    theta = i * 2 * M_PI/precision;
    glVertex2f(guardPoint2.x + r*cos(theta), guardPoint2.y + r*sin(theta));
  }
  glEnd();
}
...
void mousepress(int button, int state, int x, int y) {
  ...
  if (state == GLUT_DOWN) {
    ...
    if (poly_init_mode == 1) {
    ...
    else if (two_guard_mode == 1) {

      guardPoint2.x = mouse_x;
      guardPoint2.y = mouse_y;
      ...
      if (!isSimple(poly)) {
      ...
      else if (!isInPolygon(poly, guardPoint2)) {
        ...
        print_polygon(poly);
        triangulate2 = triangulatedPolygon(poly, guardPoint2);
      ...
      guardPoint1.x = mouse_x;
      guardPoint1.y = mouse_y;
      ...
      if (!isSimple(poly)) {
      ...
      else if (!isInPolygon(poly, guardPoint1)) {
        cout << "Point is not in the polygon" << endl;
        triangulate.clear();
        ...
        print_polygon(poly);
        triangulate = triangulatedPolygon(poly, guardPoint1);
...
void render_visible_polygon() {
  ...
  if (triangulate2.size() == 0) {
    return;
  }

  GLfloat color2[4] = {0.0, 0.0, 1.0, 0.3};

  glColor4fv(color2);

  for (int i = 0; i < triangulate2.size(); i++) {

    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    glBegin(GL_TRIANGLES);
    glVertex2f(triangulate2[i][0].x, triangulate2[i][0].y);
    glVertex2f(triangulate2[i][1].x, triangulate2[i][1].y);
    glVertex2f(triangulate2[i][2].x, triangulate2[i][2].y);
    glEnd();
