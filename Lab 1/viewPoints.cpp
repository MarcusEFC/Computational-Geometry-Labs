...
Draws a set of points in the default 2D projection and determines the convex hull.
...
int main(int argc, char** argv) {
  ...
  if (hull.size() < 3) {
    printf("Cannot calculate hull. Less than 3 points.\n");
    exit(0);
  } 
...
void draw_hull(){
  ...
  if (hull.size() >0) {
    ...
    glVertex2f(hull[hull.size() - 1].x, hull[hull.size() - 1].y); 
    glEnd(); 
